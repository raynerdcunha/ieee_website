...
export default function TopologyCopilot({ status, setStatus, sessionName, setSessionName, initialData }) {
  ...
  // 1. Listen for the initial welcome payload delivered by App.jsx
  useEffect(() => {
    if (initialData) {
      setMessages([{ 
        sender: 'system', 
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Append BOTH user and system messages using the precise clock cycles generated by the backend
      setMessages((prev) => [
        ...prev, 
      ...
      // Update both status and session name based on server state response
      if (setStatus && data.status) {
        setStatus(data.status);
      }
      if (setSessionName && data.session_name !== undefined) {
    ...
    <div className="bg-[#0b1329] rounded-xl p-4 border border-blue-900/40 shadow-xl flex flex-col h-full min-h-[600px]">
      ...
      <div className="grid grid-cols-3 items-center mb-4 w-full">
        ...
        <div className="flex justify-center">
          {/* Status Badge conditionally toggling to show Name once initialized */}
