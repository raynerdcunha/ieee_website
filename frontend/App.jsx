    ...
    <div data-theme={currentTheme} className="w-full h-screen flex flex-col bg-canvas text-primary overflow-hidden select-none" >
      ...
      <header className="w-full h-14 min-h-14 flex items-center justify-between px-6 border-b-header z-50 bg-canvas gap-4">
        ...
        <div className="flex items-center gap-4 flex-shrink-0">
          ...
          <button type="button" onClick={() => { window.history.pushState(null, '', '/'); window.location.reload(); }} className={`relative flex items-center justify-center w-8 h-8 rounded-full border-0 transition-all duration-300 cursor-pointer ${ isHistoricalMode ? 'bg-amber-500 text-black shadow-lg shadow-amber-500/20 scale-105' : 'btn-action-feature active:scale-95 hover:scale-105' }`} title={isHistoricalMode ? "Historical tracking locked - Click to escape back to present route" : "Escape back to base default route"} >
            {isHistoricalMode ? ( <Hourglass className="w-4 h-4 animate-[spin_4s_linear_infinite]" /> ) : ( <Zap className="w-4 h-4" /> )}
          </button>
        ...
        <div className="flex-1 flex items-center justify-center px-4 min-w-0">
          {isHistoricalMode ? (
            <div className="w-full max-w-2xl flex items-center justify-between px-4 py-1.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-400 font-mono text-[10px] uppercase shadow-inner animate-fade-in">
              ...
              <button type="button" onClick={() => setHistoricalMessage(null)} className="ml-4 flex items-center gap-1 font-sans font-black tracking-wide text-[9px] bg-amber-500 text-black px-2.5 py-1 rounded-lg hover:bg-amber-400 transition-all cursor-pointer active:scale-95 flex-shrink-0" >
                <RotateCcw className="w-2.5 h-2.5 stroke-[3]" /> Back to Present
              </button>
            </div>
          ) : (
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <button type="button" onClick={async () => { if (layoutMode !== "compare") { await fetchAllSessions(); setSelectTrackB(sessionName || ""); setSelectTrackC(sessionName || ""); setCompareError(""); setCompareModalOpen(true); } else { window.history.pushState(null, '', `/${sessionA || sessionName || ''}`); window.location.reload(); } }} className="flex items-center gap-1.5 px-4 py-1.5 rounded-full btn-action-feature font-mono text-[10px] md:text-xs font-bold uppercase active:scale-95 transition-all cursor-pointer" >
          ...
          <button type="button" onClick={handleResetLayout} className="flex items-center gap-1.5 px-4 py-1.5 rounded-full btn-nav-utility font-mono text-[10px] md:text-xs font-bold uppercase active:scale-95 transition-all cursor-pointer" >
            <RotateCcw className="w-3.5 h-3.5" /> <span>Reset Layout</span>
          </button>
              ...
              <div className="compare-scroll-workspace">
                <div className="w-full rounded-xl overflow-hidden h-[320px] flex-shrink-0">
                  <BusTopologyMap 
                    ...
                    onMaximizeToggle={() => setFullscreenSession({ sessionName: sessionA, stage: stageA, hasHistory: true })}
                  />
                </div>
                <div className="w-full rounded-xl overflow-hidden h-[320px] flex-shrink-0">
                  <AnalyticsStreamGallery currentTheme={currentTheme} targetSession={sessionA} historicalMessage={historicalMessage} />
                </div>
                <div className="w-full h-[440px] rounded-xl overflow-hidden relative flex-shrink-0">
                  <div style={{ width: '125%', height: '125%', transform: 'scale(0.8)', transformOrigin: 'top left' }} className="absolute inset-0">
                    <TopologyCopilot status={sessionStatus} setStatus={setSessionStatus} onStageChange={handleStageMutation} sessionName={sessionA} setSessionName={() => {}} initialData={initialDataA} currentTheme={currentTheme} hideHeader={true} onHistoricalJump={(msg) => setHistoricalMessage(msg)} historicalMessage={historicalMessage} onBranchClick={() => handleOpenBranchModal(null)} onBranchFromHistory={(msg) => handleOpenBranchModal(msg)} dashboardHistoricalMode={isHistoricalMode} />
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="compare-scroll-workspace">
                <div className="w-full rounded-xl overflow-hidden h-[320px] flex-shrink-0">
                  <BusTopologyMap 
                    ...
                    refreshTrigger={refreshTick} 
                    onMaximizeToggle={() => setFullscreenSession({ sessionName: sessionB, stage: stageB, hasHistory: false })}
                  />
                </div>
                <div className="w-full rounded-xl overflow-hidden h-[320px] flex-shrink-0">
                ...
                <div className="w-full h-[440px] rounded-xl overflow-hidden relative flex-shrink-0">
                  <div style={{ width: '125%', height: '125%', transform: 'scale(0.8)', transformOrigin: 'top left' }} className="absolute inset-0">
                    <TopologyCopilot status={"Active"} setStatus={() => {}} onStageChange={(st) => setStageB(st)} sessionName={sessionB} setSessionName={() => {}} initialData={initialDataB} currentTheme={currentTheme} hideHeader={true} onBranchClick={() => handleOpenBranchModal(null)} />
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="compare-scroll-workspace">
                <div className="w-full rounded-xl overflow-hidden h-[320px] flex-shrink-0">
                  <BusTopologyMap 
                    currentTheme={currentTheme} 
                    currentStage={stageC} 
                    targetSession={sessionC} 
                    refreshTrigger={refreshTick} 
                    onMaximizeToggle={() => setFullscreenSession({ sessionName: sessionC, stage: stageC, hasHistory: false })}
                  />
                </div>
                <div className="w-full rounded-xl overflow-hidden h-[320px] flex-shrink-0">
                ...
                <div className="w-full h-[440px] rounded-xl overflow-hidden relative flex-shrink-0">
                  <div style={{ width: '125%', height: '125%', transform: 'scale(0.8)', transformOrigin: 'top left' }} className="absolute inset-0">
                    <TopologyCopilot status={"Active"} setStatus={() => {}} onStageChange={(st) => setStageC(st)} sessionName={sessionC} setSessionName={() => {}} initialData={initialDataC} currentTheme={currentTheme} hideHeader={true} onBranchClick={() => handleOpenBranchModal(null)} />
                  </div>
                </div>
              </div>
            </div>
          ...
          <>
            <div style={{ width: `calc(${widths[0]}% - 4px)` }} className="h-full flex-shrink-0 overflow-hidden">
              <BusTopologyMap 
                ...
                onClearHistoricalView={() => setHistoricalMessage(null)} 
                onBranchFromHistory={(msg) => handleOpenBranchModal(msg)} 
                onMaximizeToggle={() => setFullscreenSession({ sessionName: sessionName, stage: sessionStage, hasHistory: true })}
              />
            </div>
            ...
            <div style={{ width: `calc(${widths[1]}% - 4px)` }} className="h-full flex-shrink-0 overflow-hidden">
              <AnalyticsStreamGallery currentTheme={currentTheme} targetSession={sessionName} historicalMessage={historicalMessage} />
            </div>
            ...
            <div style={{ width: `calc(${widths[2]}% - 4px)` }} className="h-full flex-shrink-0 flex justify-end items-end relative overflow-hidden">
              <div style={{ width: '123.333%', height: '123.333%', transform: 'scale(0.81081)', transformOrigin: 'bottom right', position: 'absolute', bottom: 0, right: 0 }}>
                <TopologyCopilot status={sessionStatus} setStatus={setSessionStatus} onStageChange={handleStageMutation} sessionName={sessionName} setSessionName={handleSessionNameChange} initialData={initialDataA} currentTheme={currentTheme} onBranchClick={() => handleOpenBranchModal(null)} onBranchFromHistory={(msg) => handleOpenBranchModal(msg)} onHistoricalJump={(msg) => setHistoricalMessage(msg)} historicalMessage={historicalMessage} dashboardHistoricalMode={isHistoricalMode} />
              </div>
            </div>
          </>
        )}
        ...
        <div className="fixed inset-0 z-[200] flex flex-col p-6 modal-overlay-bg animate-fade-in backdrop-blur-md">
          <div className="w-full flex justify-end mb-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-mono font-bold tracking-wider uppercase rounded-xl border border-red-900/30 bg-red-950/20 text-red-400 hover:bg-red-900 hover:text-white transition-all cursor-pointer shadow-lg active:scale-95"
            >
              <X className="w-4 h-4" />
              <span>Exit View Screen</span>
            </button>
          </div>
          <div className="flex-1 w-full h-full min-h-0 min-w-0 bg-canvas rounded-2xl border border-[var(--bus-border-outer)] shadow-2xl p-2 relative">
            <BusTopologyMap
              currentTheme={currentTheme}
              currentStage={fullscreenSession.stage}
              targetSession={fullscreenSession.sessionName}
              refreshTrigger={refreshTick}
              historicalMessage={fullscreenSession.hasHistory ? historicalMessage : null}
              onMaximizeToggle={() => setFullscreenSession(null)}
              isFullscreen={true}
            />
          </div>
        </div>
