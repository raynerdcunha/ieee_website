...
def generate_grid_graph(bus_system: str, ratio=0.75):
    """
    Interface function called by main.py to stream custom 
    IEEE grid coordinates dynamically.
    """
...
def main(ratio=0.75):
    ...
    r = [k for k in range(1, 38)]
    x = [k for k in range(1, 38)]
    json_graph = topology(ratio, unused_lines, v, p_load, q_load, r, x)
    return json_graph

def topology(ratio=0.75, unused_lines=None, v=None, p_load=None, q_load=None, r=None, x=None):
    if unused_lines is None: unused_lines = []
    if v is None: v = [0] * 33
    if p_load is None: p_load = [0] * 33
    if q_load is None: q_load = [0] * 33
    if r is None: r = [0] * 37
    if x is None: x = [0] * 37
    ...
    bus_coords = {
        ...
        12: (0, -2), 13: (0, -3), 14: (0, -4), 15: (0, -5), 16: (0, -6), 17: (0, -7), 18: (0, -8), 
        19: (-1, 4), 20: (-1, 3), 21: (-1, 2), 22: (-1, 1), 
        23: (1, 5), 24: (1, 4), 25: (1, 3), 
        26: (0.6, 2), 27: (0.6, 1), 28: (0.6, 0), 29: (0.6, -1), 30: (0.6, -2), 31: (0.6, -3), 32: (0.6, -4), 33: (0.6, -5),
    ...
    # 2. Define Lines
    lines = [
        ('Sub', 1, 's'), (1, 2, 's'), (2, 3, 's'), (3, 4, 's'), (4, 5, 's'), (5, 6, 's'), 
        (6, 7, 's'), (7, 8, 's'), (8, 9, 's'), (9, 10, 's'), (10, 11, 's'), (11, 12, 's'), 
        (12, 13, 's'), (13, 14, 's'), (14, 15, 's'), (15, 16, 's'), (16, 17, 's'), (17, 18, 's'), 
        (19, 20, 's'), (20, 21, 's'), (21, 22, 's'), (23, 24, 's'), (24, 25, 's'), (26, 27, 's'), 
        (27, 28, 's'), (28, 29, 's'), (29, 30, 's'), (30, 31, 's'), (31, 32, 's'), (32, 33, 's'), 
        (2191, 2192, 's'), (2192, 2193, 's'), (2193, 19, 's'), (3231, 3232, 's'), (3232, 3233, 's'), 
        (3233, 23, 's'), (6261, 6262, 's'), (6262, 6263, 's'), (6263, 26, 's'), (8211, 8212, 't'), 
        (8212, 8213, 't'), (8213, 8214, 't'), (9151, 9152, 't'), (9152, 9153, 't'), (9153, 9154, 't'), 
        (9154, 9155, 't'), (9155, 9156, 't'), (12221, 12222, 't'), (12222, 12223, 't'), (12223, 22, 't'), 
        (18, 18331, 't'), (18331, 18332, 't'), (18332, 33, 't'), (25, 25291, 't'), (25291, 25292, 't'), 
        (25292, 25293, 't'),
    ]
    ...
    line_labels = {
        ...
        (13, 14): '13', (14, 15): '14', (15, 16): '15', (16, 17): '16', (17, 18): '17', (19, 20): '19', 
        (20, 21): '20', (21, 22): '21', (23, 24): '23', (24, 25): '24', (26, 27): '26', (27, 28): '27', 
        (28, 29): '28', (29, 30): '29', (30, 31): '30', (31, 32): '31', (32, 33): '32', (2192, 2193): '18', 
        (3232, 3233): '22', (6262, 6263): '25', (8212, 8213): '33', (9153, 9154): '34', (12222, 12223): '35', 
    ...
    for (start_node, end_node), label in line_labels.items():
        if start_node in bus_coords and end_node in bus_coords:
            ...
            fig.add_trace(go.Scatter(
                ...
                hoverinfo='text', hovertext=[f"Line {label}<br>Resistance: {r[int(label)-1]}<br>Reactance: {x[int(label)-1]}"],
                hoverlabel=dict(
                    bgcolor=text_color,
    ...
    fig.add_trace(go.Scatter(
        ...
        hoverinfo='text', hovertext=[f"Bus: {k}<br>Voltage: {v[k-1]}<br>Active Load: {p_load[k-1]}<br>Reactive Load: {q_load[k-1]}" for k in real_buses],
    ...
    # --- PERMANENT FLUID STRETCH FIX: SYSTEM LINE/MARKER LOAD ARROWS ---
    # Tracks exact original side targets. Arrow length is calculated directly within data system coordinates.
    arrow_offset = 0.06
    arrow_length = 0.32
    ...
    for ax, ay in arrow_definitions:
        ...
        fig.add_trace(go.Scatter(
            x=[ax, ax],
            y=[ay + arrow_length, ay + 0.05],
            mode='lines',
            line=dict(color='black', width=2 * ratio),
            hoverinfo='none', showlegend=False
        ))
